    ...
    def evaluate(self) -> Any:
        ...
        if self.op == 'and':
            while len(self.values) != 1:
                a = self.values.pop().evaluate()
                b = self.values.pop().evaluate()
                self.values.append(Bool(b and a))
            return self.values[0].evaluate()
        else:
            while len(self.values) != 1:
                a = self.values.pop().evaluate()
                b = self.values.pop().evaluate()
                self.values.append(Bool(b or a))
            return self.values[0].evaluate()
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        """
        lst = []
        self.comparisons.append(('', self.left))
        for i in range(0, len(self.comparisons) - 1):
            if self.comparisons[i][0] == '<=':
                lst.append(Bool(self.comparisons[i - 1][1].evaluate() <= \
                                self.comparisons[i][1].evaluate()))
            else:
                lst.append(Bool(self.comparisons[i - 1][1].evaluate() < \
                                self.comparisons[i][1].evaluate()))
        return BoolOp('and', lst).evaluate()
